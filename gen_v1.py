...
OUT = "/home/claude/gerber-v1"; os.makedirs(OUT, exist_ok=True)
...
T=[
 ...
 ("VS","B",[P("U1","18"),(12.5,40.9)],TWN),      # right-edge VS pin -> escape via
 ...
 ("LDRV1","F",[(9.3,37.6),(9.3,49.0),(16.6,49.0)],TW),                               # F: straight down x9.3 (clear of UPDI via) -> south lane -> R1
 ("LDRV1","B",[(16.6,49.0),P("R1","2")],TW),                                         # via drop -> R1.2
 # NOTE: LDRV2/3/4 deferred. They reverse vs the ballasts (pin x-order opposite ballast x-order), which forces the bottom-edge escapes + descents into sub-0.15mm spacings that don't fit (proven: routes connect but graze C2/C5/each other). The clean fix is the schematic pin reassignment (pin26->LED4 ... pin1->LED1, firmware-interchangeable) so they NEST - a netlist change for Devin, not a generator-only nudge (relabeling nets here alone would mismatch the schematic = fab footgun). Once reassigned: F-descend-past-components -> drop S of the ballast row -> nested B return, ~2 vias each, no dips.
 # UPDI: fine via -> F T-junction (north to TC1.1, west to J1.1)
 ("UPDI","B",[P("U1","23"),(10.5,37.6)],TWN),                                        # dog-bone NE -> outer-row fine via
 ("UPDI","F",[(10.5,37.6),(10.9,37.6),(10.87,15.63)],TW),                            # F north up x10.9 to TC1.1 (via-in-pad)
 ("UPDI","F",[(10.5,37.6),(9.0,36.8),(4.7,36.8)],TW),                                # F west (below LDRV1 via) to J1 column
 ("UPDI","B",[(4.7,36.8),P("J1","1")],TW),                                           # via drop -> J1.1
...
VIAS += [("VS",14.8,47.6,"B"),("VS",21.1,47.6,"B"),("VS",27.4,47.6,"B"),("VS",33.4,47.6,"B"),   # LED-anode VS stitch
         ("GND",8.9,40.9,"B"),("GND",10.1,40.9,"B"),                                             # U1 EP GND stitch vias
         ("VS",12.5,40.9,"B"),("VS",9.9,38.3,"B"),                                               # U1 VS escape vias (bottom = fine)
         ("GND",40.4,73.5,"F"),                                                                   # SW1 dome GND return -> In1 (on the horseshoe, clear of BTN)
         ("VIN",44.8,43.5,"FB"),                                                                  # VIN F->B at D1.A
         ("LDRV1",9.3,37.6,"FB"),("LDRV1",16.6,49.0,"FB"),                                        # LDRV1 fine escape + drop at R1.2
         ("UPDI",10.5,37.6,"FB"),("UPDI",10.87,15.63,"FB"),("UPDI",4.7,36.8,"FB"),                # UPDI fine escape + drops at TC1.1 / J1.1
...
FINE.update({(9.9,38.3),(9.3,37.6),(10.5,37.6),(6.0,40.1),(6.0,41.3),(12.5,40.9),(12.0,44.5),(12.5,44.0)})  # QFN-edge + left-escape + pin18-VS + SDA/SCL-transition fine vias
...
def build_copper(grow=0.0):
    ...
    for net,x,y,lands in VIAS:                  # land only where it connects (stitch via = its pad side)
        for L in (("F","B") if lands=="FB" else (lands,)): add(net,L,via_poly(x,y,grow))
    return {k:unary_union(v) for k,v in cu.items()}

# ===================== inner planes (In1=GND, In2=VS) with antipads =====================
def build_pours():
    ...
    for net,x,y in THRU:
        ...
        if net!="GND": in1=in1.difference(c)    # off-net via -> antipad on GND plane
        if net!="VS":  in2=in2.difference(c)    # off-net via -> antipad on VS plane
    for x,y,d in nonplated:                      # non-plated holes antipad both
        c=Point(x,y).buffer(d/2+POUR_CLR,resolution=20); in1=in1.difference(c); in2=in2.difference(c)
    # also clear the planes away from F/B signal traces' vias already handled; SMD pads don't reach inner layers
...
_p=build_pours()
print(f"In1 GND plane area={_p['In1'].area:.0f}mm2   In2 VS plane area={_p['In2'].area:.0f}mm2")
...
In1, In2 = pours["In1"], pours["In2"]
# soldermask: open over pads on each side + the bare-FR4 glow window
Fpads=unary_union([get_pad_poly(r,p,x,y,w,h,sh,rt) for r,p,n,x,y,w,h,sh,rt,s in abs_pads if s=="F"]+[via_poly(x,y) for n,x,y,lands in VIAS if "F" in lands])
Bpads=unary_union([get_pad_poly(r,p,x,y,w,h,sh,rt) for r,p,n,x,y,w,h,sh,rt,s in abs_pads if s=="B"]+[via_poly(x,y) for n,x,y,lands in VIAS if "B" in lands])
...
Fcu_o,Bcu_o,In1_o,In2_o,Fmask_o,Bmask_o,OUT_o=(fy(Fcu),fy(Bcu),fy(In1),fy(In2),fy(Fmask),fy(Bmask),fy(OUTLINE))
...
emit_fill(f"{B}.GL2", In1_o, "Copper,L2,Inr")    # GND plane
emit_fill(f"{B}.GL3", In2_o, "Copper,L3,Inr")    # VS plane
emit_fill(f"{B}.GBL", Bcu_o, "Copper,L4,Bot")
...
try:
    ...
    for f in ["GTL","GL2","GL3","GBL","GTS","GBS","GKO"]:
...
panels=[("F.Cu (GTL)",Fcu_o,"#c9a227"),("In1 GND (GL2)",In1_o,"#6b9bd1"),("In2 VS (GL3)",In2_o,"#d98a4f"),("B.Cu (GBL)",Bcu_o,"#c9a227")]
...
fig.suptitle("SOLAR-GLOW DRH v1 - 4-layer (Y-up), planes + pads + power stitch + SW1 dome + front QR",color="#d9a23a",fontsize=12,y=0.97)
fig.tight_layout(rect=[0,0,1,0.95]); fig.savefig("/mnt/user-data/outputs/solar-glow-drh-v1-gerber-preview.png",dpi=150,facecolor="#0a0a0c")
print("saved preview")
...
def connectivity():
    ...
    for gg in _explode(pours["In2"]): pieces["VS"].append(("In2",gg))
    # bridges: vias (F+B, plus its plane if GND/VS) and plated MH (GND thru: F+B+In1)
    bridges=[(n,x,y,(("F","B") if lands=="FB" else (lands,))+(("In1",) if n=="GND" else ())+(("In2",) if n=="VS" else ())) for n,x,y,lands in VIAS]
    bridges+=[("GND",x,y,("F","B","In1")) for x,y,d in plated]
    rep={}
...
def shorts():
    ...
    for (n,s),g in cu.items():
        ...
        lay[s].append((n,g))
    lay["In1"].append(("GND",pours["In1"])); lay["In2"].append(("VS",pours["In2"]))
    viol=[]
