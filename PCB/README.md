...
| NFC | NT3H2211 tag (U5) + **PCB-etched 7-turn coil** on B.Cu, power-gated by U6. The coil is **exposed through B.Mask** (2026-07-29), so the spiral plates ENIG and reads as gold on black |
...
- **The NFC antenna is etched copper** — no wound coil to buy. It is also *visible*: the
  soldermask is opened over it, so the etch is the artwork. That aperture is generated by
  `scripts/mask_art.py` from the LA/LB copper itself (never drawn), it exposes those two
  nets and nothing else, and it clears the grounded titanium lip by 0.95 mm and the eight
  boss annuli by 0.91 mm. Opening mask over the coil slightly lowers inter-turn
  capacitance, which nudges the tune — it lands in the retune C9 is already reserved for.
