...
- **A contactless mark sits on the show face**, over the antenna at x[41.3,44.1] y[42.2,46.7] —
  generated by `scripts/mask_art.py`, subject to the same `(glyph − live copper)` rule as the
  cartouche, so it can only ever lay GND or bare laminate bare. It falls inside the coil keepout
