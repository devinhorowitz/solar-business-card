...
- **Zero idle draw, for free.** The board already power-gates the NFC tag with a TPS22917 load
...
- **Power-gate:** own load switch on a spare GPIO (the same TPS22917 load-switch power-gating the board already uses for the NFC tag and the now-committed v4 FRAM on the shared VNFC rail), EPD fully off between
