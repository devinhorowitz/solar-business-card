...
| Mounting holes | **8× M2, GND** -- 4 corner (MH1-4) at **(2.87, 2.87) / (47.93, 2.87) / (2.87, 86.03) / (47.93, 86.03)** (pitch **45.06 × 83.16 mm**) + 4 panel-corner (MP1-4) at **(2.87, 28.63) / (47.93, 28.63) / (2.87, 60.27) / (47.93, 60.27)** | corners concentric with the r3.0 fillets; MP1-4 at the E/W mid-edges for the shell clamp |
...
| BOM | **generated from the design** — there is no hand-authored master; most passives 0402 with the precision/bulk set on **0603** (C4/C13/C25/C22/C23/R5/R6/R15/R16) and **0805** (C26/C27); **SJ1 removed outright 2026-07-30** (lineage row only); Q2 + R18 added by the cold-start-deadlock fix | line items come from `scripts/bom_split.py` (schematic + the board's own `exclude_from_bom`/`dnp` flags); the buy documents are written into `Generated/fabdocs/` every CI run; live availability + pricing in `BOM/README.md` (`python3 BOM/check_stock.py`). Both `*-BOM.xlsx` masters were culled 2026-08-02 and live in git history |
...
| Enclosure geometry | `enclosure/solar-glow-drh-v4_0-backshell-0p6b-brace-cad.py` (prints the Z-stack; regenerates the STEP) |
...
| BOM | **the design itself** — `scripts/bom_split.py` derives every line from the schematic plus the board's `exclude_from_bom`/`dnp` flags, and writes the two buy documents into `Generated/fabdocs/` on each CI run. `BOM/README.md` is **derived too**: a live stock/lifecycle/price table regenerated by `BOM/check_stock.py` (needs distributor API keys, so manual-apply like the mask art). _(There is no spreadsheet master; both `*-BOM.xlsx` files were culled 2026-08-02 — git history only.)_ |
...
- **Stores** energy in **four** series-parallel supercapacitors -- a **hybrid tank** (two larger
  SS17 cells + two WS17), **1.40 F at 5.5 V** — two EQUAL 2.8 F stages (SC1∥SC2 above MID, SC3∥SC4
  below), **≈ 21 J nameplate but 15.1 J stored and 9.8 J spendable** once the AEM's strapped 4.65 V
  `VOVCH` ceiling and the 2.75 V glow floor are applied — with the AEM10300 harvester (U8) balancing
  the series midpoint (MID) against leakage mismatch between those two identical stages; the safe 3.3 V VS rail
...
average several milliamps. The two-panel harvest and the tank — **9.8 J spendable**, not the 21 J
nameplate — are sized to **harvest
...
│   ├── README.md                       # GENERATED live availability/price table (check_stock.py)
...
│   ├── thomsonlint.md                   # the ontology-guided second reviewer: pinned run recipe + the 2026-08-01 review record
...
- **Per-board parts cost: see [`BOM/README.md`](BOM/README.md)**, the live table — and the **four supercaps are the dominant line** —
