...
# ---- the BACK plot, for the views that only ever see the back ---------------------------
# This animation looks up from under the horizon, so the face in frame is the board's
# UNDERSIDE -- and vtkTextureMapToPlane projects along Z, which put the FRONT artwork on both
# faces of the prism. The card's front then appeared on its own back, reading mirrored the way
# print does through a substrate, and the stack looked as though the show face were sandwiched
# inwards. That was the complaint, and the fix is not to fight the projection or spin the
# camera to hunt for the front: the front is never in frame here, so texture the back with the
# BACK.
#
# Mirrored on load, deliberately. The front mapping was calibrated for a face seen from +Z; a
# face seen from -Z has the opposite handedness, so the plot needs one flip to come back the
# right way round. Doing it on the IMAGE rather than in the u/v corners leaves board_pd's
# single tcoord array alone, which the card flip still needs for the front.
TEX_BACK = f"{ROOT}/Generated/docs/solar-glow-drh-v4_0-card-back.png"
if not os.path.exists(TEX_BACK):
    raise SystemExit(f"missing {TEX_BACK} -- generated by the KiBot workflow on PCB/** changes")
_tb = vtk.vtkPNGReader(); _tb.SetFileName(TEX_BACK); _tb.Update()
_tbf = vtk.vtkImageFlip(); _tbf.SetInputConnection(_tb.GetOutputPort())
_tbf.SetFilteredAxis(0); _tbf.Update()
board_tex_back = vtk.vtkTexture(); board_tex_back.SetInputConnection(_tbf.GetOutputPort())
board_tex_back.InterpolateOn(); board_tex_back.EdgeClampOn(); board_tex_back.MipmapOn()


def textured_back(a):
    """The board wearing its own B-side, for any view shot from under the horizon."""
    a.GetProperty().SetColor(1.0, 1.0, 1.0)
    a.SetTexture(board_tex_back)
    return a
...
groups["board"] = ([textured_back(actor(board_pd, MASK, spec=0.30, power=26))]
...
ren2.AddActor(textured_back(_mat(board_pd, MASK, 0.85, 0.30, 26, 0.14)))
for _p in solar_pds:
