...
- **Rails run the full length (y2.10–86.80)** outboard of the supercaps (SC1/SC3 x7–24, SC2/SC4 x26.8–43.8, **0.75 mm gap since 2026-08-23** — the caps are HAND-SOLDERED, so they get a per-part in-plane clearance (`fit_rules.CLR_EXCEPTIONS`) rather than the 0.25 mm every machine-placed part gets. Rotation is the binding slop term: 1.5° on a 39 mm cap swings its corner 0.51 mm. Cost was 111 mm² of footprint; the narrowest web (west rail) went 2.29 → 1.79 mm and it stays one piece). The caps sit in the two open bays; the rails back the four PV solder tabs.
