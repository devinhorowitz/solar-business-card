# SOLAR-GLOW DRH v2.1 — Titanium Back-Shell
## CNC Fabrication Notes / Drawing Callouts (PCBWay)

Attach this sheet to the CNC quote alongside the STEP model. The 3D model governs all
geometry; this sheet calls out material, the few dimensions that need tighter-than-standard
control, and the items a titanium shop will otherwise raise as an engineering query (EQ).
...
| Source model | `solar-glow-drh-v2_1-backshell-Ti-max.step` (0.55 floor) — or `-Ti-conservative.step` (0.60 floor); see Thin-Wall Advisory |
| Units | mm |
| Drawing | None; STEP governs. Critical dims and notes below. |

The mating PCB and the 4 fasteners are **separate, customer-supplied** parts, not part of this order.

---

### 1. Overall dimensions and datum

- Bounding box (Ti-max / 0.55 floor): **52.70 × 90.80 × 3.35 mm**. (Ti-conservative / 0.60 floor: 53.90 × 92.00 × 3.40 mm.)
...
- Wall 1.00 mm, perimeter lip 1.50 mm, 2 cap-gap ribs 1.00 mm wide, back-frame step 0.15 mm.

---

### 2. Critical dimensions — flag these on the order for tighter control

Default everything to ISO 2768-1 general tolerance. Control **only** the four items below; each
is a function-critical fit. (Per PCBWay, tighter tolerances need to be identified for production
management and are priced per flagged location, so we flag the minimum set.)
...
| C2 | PCB-rest plane flatness (lip + 4 bosses + 2 rib tops, all coplanar at Z +2.40) | — | flatness **0.05 mm** | Board must seat flat on all rests so the screws clamp evenly. |
...
The cavity floor is **0.55 mm** on the Ti-max model (0.60 mm on the conservative model). A 0.25 mm
hairline reflector groove (note 9) cuts 0.05 mm into the cavity face, so the **thinnest section is
0.50 mm**, which sits right at the bare-minimum achievable metal wall (~0.5 mm). This is still below
the general metal minimum-wall guidance (~0.8 mm) and the titanium-specific minimum wall (~1.0 mm,
with ~1.5 mm ideal), because thin titanium flexes and chatters during cutting.
...
- **(B)** Quote the 0.60 mm conservative floor instead (`-Ti-conservative.step`).

**Question for the shop:** what is the minimum floor thickness you will reliably hold in
Ti-6Al-4V for this ~48 × 86 mm pocket, given the rib backing? We will set the floor to your answer.
...
- Note: M2 coarse pitch is 0.4 mm, below the 0.6 mm minimum-pitch gate on the online quote form.
  Please tap M2 per this note (or advise) rather than letting the auto-checker reject the thread.
- Customer-supplied fasteners: 4× **brass M2 × 3 mm pan head**, head ⌀ ≤ 4.0 mm. Tip seats flush in the back spotface (note 5).
...
No internal corner in the model is sharper than the finishing tool. All concave junctions are
pre-radiused:
...
- Back recessed field (0.15 mm deep): annulus-to-frame junctions radiused **R0.5** (≤⌀1.0 mm finisher; shallow, so reach is trivial).
...
- Machine it as modeled (a light engrave with a fine bit), **or** laser-mark the same outline
  instead (zero material removed, floor stays 0.55 mm full). Either is acceptable; the as-modeled
  cut is the worst case for wall thickness.
...
- `solar-glow-drh-v2_1-backshell-Ti-max.step` (0.55 floor) **or** `...-Ti-conservative.step` (0.60 floor)
- This notes sheet.
