...
- **Inserts: No.**
- **Tolerance: leave on standard / ISO 2768** — do **not** enable “Tighter tolerances required.” That toggle trips an automated review gate that rejected the order four times with an identical templated “tighter tolerance not specified at position” message even though the drawing marks it in four places. Marked callouts govern regardless of the toggle: a dimension toleranced ±0.05 on the drawing is held to ±0.05, and the general 2768 setting applies only to **unmarked** dimensions. Two dimensions are marked **±0.05** at their feature (in note 3, the critical-dims table, and the title-block tolerance block): **C1 cavity depth 1.90 ±0.05** (Section A-A) and **C3 mounting-hole pattern pitch 43.80 / 82.90 ±0.05** (plan). **C1 is the non-negotiable one** — under 1.85 the floor presses on U2 (1.90 ±0.05 = range 1.85–1.95, gap 0.10–0.20). Flatness C2 = 0.05 rides along as a form callout. Paste into the order’s special-request / notes box: *“Two dimensions are marked ±0.05 on the drawing and must be held as marked: cavity depth 1.90 ±0.05 (Section A-A), and mounting-hole pattern pitch 43.80 / 82.90 ±0.05 (plan). All other dimensions per ISO 2768-1 medium.”*
