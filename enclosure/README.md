...
> **Source of truth.** `solar-glow-drh-v4_0-backshell-0p6b-brace-cad.py` is authoritative for all
...
| File | Purpose |
|---|---|
| `solar-glow-drh-v4_0-backshell-0p6b-brace-cad.py` | Parametric CadQuery generator, **all three shells per run**. **Source of truth** — regenerates the STEP/STLs from the PCB anchors + `fit_rules.VARIANTS`. |
| `solar-glow-drh-v4_0-backshell-0p6b-brace-Ti-max.step` | **Send this to the fab for a MAX build.** 1.00 floor (**true uniform** — the U7 relief pocket was removed 2026-07-28), 1.80 cavity, 0.60 board recess, no ribs, no locator pillars, 1.0 walls, asymmetric lip (W2.5/N2.0/S2.0/E1.0), 8 M2 bosses, 3.55 overall. Lite/air fab files: the variant STEP + its own drawing, same rule. |
| `solar-glow-drh-v4_0-backshell-0p6b-brace-Ti-max.stl` | Same geometry, for a quick plastic dry-fit print before committing to titanium. |
| `solar-glow-drh-v4_0-backshell-0p6b-brace-DRAWING.pdf` / `.png` | 2D dimensioned drawing (plan + Section A-A + Detail B + critical dims + notes + title block). **Regenerated by CI with the CAD in the same commit** on any board or generator change (`kibot.yml`; the generators themselves are triggers since 2026-07-31), so it cannot lag the STEP — the 2026-07-28 stale-pocket episode (§7) is the failure that arrangement closed. The current sheet carries the medallion-era rear-art block. |
...
`solar-glow-drh-v4_0-backshell-0p6b-brace-DRAWING.pdf` matches the committed STEP on every headline
...
| Source model | `solar-glow-drh-v4_0-backshell-0p6b-brace-Ti-max.step` |
| Units | mm |
